...
def run(drive , system , skip_conf):
    ...
    try:
        ...
        for syskey in keys_to_process:
            ...
            #if system folder doesnt exist , just continue
            roms_p = f"{drive}/{syskey}"
            if not os.path.isdir(roms_p):
                print(f"Rom folder {syskey} not found")
                continue
